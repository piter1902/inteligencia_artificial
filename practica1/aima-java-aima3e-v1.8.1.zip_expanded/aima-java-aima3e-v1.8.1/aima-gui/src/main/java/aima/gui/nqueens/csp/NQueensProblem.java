...
public class NQueensProblem extends CSP {
     ...
     private static List<Variable> collectVariables() {
          ...
          for (int i = 0; i < 9; i++)
               for (int j = 0; j < 9; j++) {
                    variables.add(new NQueensVariable("Cell at [" + i + "][" + j
                              + "]", i, j));
               }
          return variables;
     }
      ...
      */
     private static List<Integer> getSudokuDomain(NQueensVariable var) {
      ...
      * @param pack
      */
     public NQueensProblem(AvailableCells pack) {
    	     //variables
          ...
          for (Variable var : getVariables()) {
               domain = new Domain(getSudokuDomain((NQueensVariable) var));
