...
public class EightPuzzlePract2 {
	...
	public static int generar100busquedas(Search search, int depth) {
		...
		for (int i = 0; i < 100; i++) {
			// Comprobamos que la solucion esta a profundidad depth mediante el algoritmo A* (con Heuristica Manhattan)
			EightPuzzleBoard initial = null;
			int prof_result = 0;
			do {				
				// Generamos un tablero y ponemos su solucion
				initial = GenerateInitialEightPuzzleBoard.randomIni();
				...
				Problem test = new Problem(new EightPuzzleBoard(initial), EightPuzzleFunctionFactory.getActionsFunction(),
						EightPuzzleFunctionFactory.getResultFunction(), EightPuzzleGoalTest2.getGoal());
				SearchAgent sa = null;
				try {
					sa = new SearchAgent(test, new AStarSearch(new GraphSearch(), new ManhattanHeuristicFunction2()));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				if(sa != null) {
					prof_result = (int) Float.parseFloat(sa.getInstrumentation().getProperty("pathCost"));
				}
			}while(prof_result != depth);
