...
public class IterativeDeepeningSearch extends NodeExpander implements Search {
	...
	public IterativeDeepeningSearch() {
		...
		iterationMetrics.set(NODES_GENERATED, 0);
	...
	public List<Action> search(Problem p) throws Exception {
		...
		for (int i = 0; i <= infinity; i++) {
			...
			iterationMetrics.set(METRIC_NODES_EXPANDED,
					iterationMetrics.getInt(METRIC_NODES_EXPANDED) + dls.getMetrics().getInt(METRIC_NODES_EXPANDED));
			iterationMetrics.set(NODES_GENERATED,
					iterationMetrics.getInt(NODES_GENERATED) + dls.getMetrics().getInt(NODES_GENERATED));
			// if result != cutoff then return result
